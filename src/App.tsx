...
import { Sun, Moon, Languages, Phone, Facebook, MessageCircle, Info, Hammer, Paintbrush, ShieldCheck } from 'lucide-react';
...
const App = () => {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${theme === 'dark' ? 'bg-zinc-950 text-zinc-100' : 'bg-zinc-50 text-zinc-900'}`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      {/* Navbar */}
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? (theme === 'dark' ? 'bg-zinc-950/80 backdrop-blur-md py-4' : 'bg-white/80 backdrop-blur-md py-4 shadow-sm') : 'bg-transparent py-6'}`}>
        <div className="container mx-auto px-6 flex justify-between items-center">
          ...
          >
            <div className="w-10 h-10 bg-red-600 rounded-lg flex items-center justify-center text-white shadow-lg shadow-red-600/20">
              <ShieldCheck size={24} />
            </div>
            <span className="hidden sm:inline">{t.common.logo}</span>
          </motion.div>

          <div className="flex items-center gap-4">
            <button onClick={toggleLang} className="p-2 rounded-full hover:bg-zinc-500/10 transition-colors flex items-center gap-2">
              <Languages size={20} />
            ...
            <a href="https://wa.me/971554661718" target="_blank" rel="noreferrer" className="hidden md:flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-5 py-2 rounded-full transition-all shadow-lg shadow-red-600/20 font-medium">
              <Phone size={18} />
        ...
        <div className="container mx-auto px-6 relative z-10">
          ...
          >
            <motion.span 
              ...
              className="inline-block px-4 py-1 rounded-full bg-red-600/10 border border-red-600/20 text-red-600 text-sm font-bold tracking-widest uppercase mb-6"
            >
              {t.hero.badge}
            </motion.span>
            ...
            <div className="flex flex-wrap gap-4">
              <motion.a 
                ...
                whileTap={{ scale: 0.95 }}
                className="bg-red-600 hover:bg-red-700 text-white px-8 py-4 rounded-xl font-bold flex items-center gap-3 shadow-2xl shadow-red-600/30 transition-all cursor-pointer"
              >
              ...
              <motion.a 
                ...
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="bg-zinc-500/10 hover:bg-zinc-500/20 backdrop-blur-md px-8 py-4 rounded-xl font-bold transition-all border border-zinc-500/20 cursor-pointer"
              >
                {t.nav.services}
              </motion.a>
            </div>
      ...
      <section className={`py-12 border-y ${theme === 'dark' ? 'border-zinc-900 bg-zinc-950' : 'border-zinc-200 bg-white'}`}>
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
              { label: t.values.quality, icon: <ShieldCheck className="text-red-600" /> },
              { label: t.values.expert, icon: <ShieldCheck className="text-red-600" /> },
              { label: t.values.fast, icon: <ShieldCheck className="text-red-600" /> },
              { label: t.values.fair, icon: <ShieldCheck className="text-red-600" /> },
            ].map((v, i) => (
              <motion.div 
                key={i}
              ...
              >
                {v.icon}
                <span className="font-bold text-sm tracking-widest uppercase">{v.label}</span>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section id="services" className="py-24 relative overflow-hidden">
        <div className="container mx-auto px-6">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-4">
            <div>
              <h2 className="text-4xl font-bold tracking-tight mb-4">{t.services.title}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <ServiceCard 
              ...
              image="/images/hero_car_dubai.png" // Placeholder or similar
              theme={theme}
            />
            <ServiceCard 
      ...
      {/* About Section */}
      <section id="about" className={`py-24 ${theme === 'dark' ? 'bg-zinc-900/50' : 'bg-zinc-100'}`}>
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            >
              ...
              <div className="aspect-[4/5] rounded-[2rem] overflow-hidden shadow-2xl relative">
                <motion.div 
                  ...
                  style={{ width: "800%" }} // 8 images * 100% each
                >
                  {[
                    ...
                    <div key={idx} className="w-[12.5%] h-full flex-shrink-0">
                  ...
                  {[
                    ...
                    <div key={`dup-${idx}`} className="w-[12.5%] h-full flex-shrink-0">
                      <img src={img} alt={`Gallery-dup ${idx}`} className="w-full h-full object-cover" />
                    </div>
                  ))}
                </motion.div>
              ...
              <div className="absolute -bottom-10 -right-10 w-48 h-48 bg-red-600 rounded-3xl p-8 flex flex-col justify-end text-white shadow-2xl hidden md:flex z-20">
                <span className="text-4xl font-bold">{t.common.years}</span>
                <span className="text-sm font-medium opacity-80 uppercase tracking-tighter">{t.common.yearsExperience}</span>
              </div>
            </motion.div>
            ...
            >
              ...
              <h3 className="text-xl text-red-600 font-bold mb-8">{t.about.owner}</h3>
              ...
              <ul className="space-y-4 mb-10">
                {t.about.features.map((item, i) => (
                  <li key={i} className="flex items-center gap-3">
                    <div className="w-6 h-6 rounded-full bg-red-600/20 flex items-center justify-center text-red-600">
                      <ShieldCheck size={14} />
                    </div>
              ...
              <button className="text-lg font-bold border-b-2 border-red-600 pb-1 hover:text-red-600 transition-colors">
                {t.about.learnMore}
              </button>
            </motion.div>
          </div>
